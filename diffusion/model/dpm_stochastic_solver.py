...
class DPM_Stochastic_Solver:
    ...
    def dpm_solver_first_update(self, x, s, t, zs=None, model_s=None, return_intermediate=False):
        """
        ...
        if self.algorithm_type == "dpmsolver++":
            phi_1 = torch.exp(-h)
            phi_2 = -torch.expm1(-2*h)
            if model_s is None:
                model_s = self.model_fn(x, s)
            x_t = sigma_t / sigma_s * phi_1 * x + alpha_t * phi_2 * model_s + sigma_t * torch.sqrt(phi_2) * zs
            ...
            if model_s is None:
                model_s = self.model_fn(x, s)
            x_t = torch.exp(log_alpha_t - log_alpha_s) * x - (2 * sigma_t * phi_1) * model_s + sigma_t * torch.sqrt(phi_2) * zs

            return x_t

    def multistep_dpm_solver_second_update(self, x, model_prev_list, t_prev_list, t, zs=None, solver_type="dpmsolver"):
        """
        ...
        r1 = (lambda_prev_1 - lambda_prev_0) / h
        D1 = (model_prev_1 - model_prev_0) / r1

        if self.algorithm_type == "dpmsolver++":
            phi_1 = torch.exp(-h)
            phi_2 = -torch.expm1(-2*h)
            
            x_t = (sigma_t / sigma_prev_0) * phi_1 * x + (alpha_t * phi_2) * model_prev_0 + 0.5 * (alpha_t * phi_2) * D1 + sigma_t * torch.sqrt(phi_2) * zs
            ...
            x_t = (
                ...
                - 2 * (sigma_t * phi_1) * model_prev_0
                - (sigma_t * phi_1) * D1
                + sigma_t * torch.sqrt(phi_2) * zs
            )

        return x_t

    def multistep_dpm_solver_update(self, x, model_prev_list, t_prev_list, t, order, zs=None, solver_type="dpmsolver"):
        """
        ...
        """
        if order == 1:
            return self.dpm_solver_first_update(x, t_prev_list[-1], t, zs=zs, model_s=model_prev_list[-1])
        elif order == 2:
            return self.multistep_dpm_solver_second_update(x, model_prev_list, t_prev_list, t, zs=zs, solver_type=solver_type)
        else:
